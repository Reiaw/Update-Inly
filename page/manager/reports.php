...
if ($start_date && $end_date) {
    switch ($report_type) {
        case 'inventory':
            $query = "SELECT p.product_id, pi.product_name, p.expiration_date, 
        ...
        case 'withdrawals':
            $query = "SELECT w.withdraw_id, w.withdraw_quantity, w.transaction_date,
                             u.name, u.surname, pi.product_name
            ...
            break;
        case 'orders':
            $query = "SELECT o.order_id, o.order_status, o.total_amount,
                      ...
                      AND o.store_id = ?  -- แก้ไขเงื่อนไขตรงนี้เพื่อให้ใช้ o.store_id แทน
                      GROUP BY o.order_id
        ...
        case 'damaged_products':
            $query = "SELECT dp.deproduct_id, dp.product_id, pi.product_name, 
                                dp.deproduct_type, dp.created_at,
            ...
            break;
        case 'notifications':
            $query = "SELECT np.notiflyproduct_id, pi.product_name, 
    ...
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ssi", $start_date, $end_date, $store_id);
    $stmt->execute();
    $result = $stmt->get_result();
            ...
            <?php if (!empty($report_data)): ?>
                ...
                <div class="table-responsive">
                    ...
                    <?php elseif ($report_type == 'orders'): ?>
                        ...
                        <?php elseif ($report_type == 'order_details'): ?>
                            <!-- Order Details Table -->
                        <table id="reportTable" class="table table-striped table-bordered">
                            <thead>
                                <tr>
